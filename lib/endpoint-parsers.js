...
const parseIdAndNames = (data) => {
  const fieldNames = ['Id', 'Name', 'SingularName', 'PluralName', 'DefinitiveName', 'FriendlyId']
...
const parseGeoPosition = (data) => {
  return extractAndCamelizeData(data, ['X', 'Y'])
}
...
const parseFile = (data) => {
  ...
  const fieldNames = ['ContentType', 'Description', 'FriendlyName', 'ImageAspectRatio', 'Length']

  return {
    ...parseIdAndNames(data),
    ...parseRecordTimestamps(data),
    ...extractAndCamelizeData(data, fieldNames),
    imageSize: extractAndCamelizeData(imageSize, ['Height', 'Width']),
  }
}
...
//   Name(string)
//   Uri(string)
const parseServiceGuideService = (data) => {
