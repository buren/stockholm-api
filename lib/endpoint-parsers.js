...
const camelizeData = (data, fieldNames) => {
  if (Array.isArray(data)) {
    return data.map((object) => camelizeObjectKeys(object, fieldNames))
  }
...
const parseCount = (data) => {
  return camelizeData[data, 'Value']
}
...
const parseGeoPosition = (data) => camelizeData(data, ['X', 'Y'])

// FileInfos(array)
//   Id (uuid)
//   Name (String)
const parseFileInfo = (data) => camelizeData(data, ['Id', 'Name'])
...
const parseFile = (data) => {
  return {
    ...
    imageSize: camelizeData(data['ImageSize'] || {}, ['Height', 'Width']),
  }
}
...
//   Id(integer)
//   Name(string)
const parseGeoAreaInfo = (data) => camelizeData(data, ['FriendlyId', 'Id', 'Name'])
...
const parseGeoArea = (data) => {
  ...
  return camelizeData(data, fieldNames)
}
...
const parseDetailedServiceUnit = (data) => {
  ...
  return {
    ...
    attributes: attributes.map((attribute) => {
      return camelizeData(attribute, attributeFields)
    }),
    geographicalAreas: geographicalAreas.map((geographicalArea) => {
      return camelizeData(geographicalArea, geoFields)
    }),
    ...
    serviceUnitTypes: serviceUnitTypes.map((serviceUnitType) => {
      ...
      return {
        ...camelizeData(serviceUnitTypeFields, serviceUnitTypeFields),
...
//   Name(string)
//   Uri(string)
const parseServiceGuideService = (data) => camelizeData(data, ['Name', 'Uri'])
