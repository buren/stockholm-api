...
const buildResponse = (response) => {
  return {
    response: response,
    parse: function() { parser(this.response.json()) },
  }
}
...
class StockholmAPI {
  ...
  getService(service, params) {
    ...
    throw new Error(`argument error - either no argument or one of ${endpoint.filters.join(', ')} must be present`)
  }

  GET(url, parser) {
    if (!url)    throw new Error('url must be present!')
    if (!parser) throw new Error('parser must be present!')

    return fetch(url).then((response) => buildResponse(response))
  }
